...
import { auth, GoogleAuthProvider, createUserWithEmailAndPassword, signInWithEmailAndPassword, signInWithPopup, setDoc, doc, firestore, database, set, ref, onValue  } from '../firebase/firebaseConfig';
...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      if (!passwordPattern.test(password)) {
        setRegistrationMessage("Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, and one number.");
        return;
      }
      ...
      try {
        if (isSignUp) {
          ...
          if (!phoneNumberPattern.test(phoneNumber)) {
            setRegistrationMessage("Phone number must be 10 digits and start with 07.");
            return;
          }
